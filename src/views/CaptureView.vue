...
async function save(show = true) {
  ...
  const editingExisting = Boolean(folderName.value || route.query.draft);
  ...
  try {
    ...
    // Keep every managed image at the top of the article. Remove any existing
    // copy first so saving never duplicates a reference in the body.
    const bodyWithoutImageRefs = imageRefs.reduce((value, ref) => value.replaceAll(ref, ""), body).replace(/\n{3,}/g, "\n\n").trim();
    const sourceImages = images.value.map(image => image.relativePath).filter((path): path is string => Boolean(path));
    article.value = await saveArticle(folderName.value, { title: title.value.trim(), sourceUrl: sourceUrl.value.trim(), content: [...imageRefs, bodyWithoutImageRefs].filter(Boolean).join("\n\n"), status: "active", captureStep: 3, sourceImages });
    if (show) {
      if (editingExisting) successMessage.value = "资料已保存。";
      else {
        successMessage.value = "资料已保存到资料库。";
...
<template>
  <section class="page-view capture-view"><div class="page-heading"><div><button class="editor-back" type="button" @click="router.push('/library')"><ArrowLeft :size="16" />返回资料库</button><h2>{{ folderName ? '编辑资料' : '新建资料' }}</h2></div><button class="button button-primary" :disabled="!canSave" @click="save()"><LoaderCircle v-if="saving" class="spin" :size="16" /><Save v-else :size="16" />{{ saving ? '保存中…' : '保存资料' }}</button></div>
    <div v-if="loading" class="capture-loading">正在读取资料…</div><form v-else class="capture-form" @submit.prevent="save()"><label>标题 <input v-model="title" required maxlength="120" placeholder="请输入资料标题" /></label><label>来源链接 <span class="input-icon"><Link2 :size="15" /><input v-model="sourceUrl" type="url" placeholder="https://（可选）" /></span></label><section class="image-panel"><div class="image-panel-head"><strong>正文配图</strong><span>{{ images.length }} 张</span></div><div class="dropzone" @dragover.prevent @drop="onDrop"><Upload :size="22" /><span>将图片拖入此处，或</span><button class="button button-secondary" type="button" :disabled="adding" @click="chooseImages"><ImagePlus :size="16" />选择图片</button><button class="button button-secondary" type="button" :disabled="adding" @click="pasteImages">粘贴图片</button><small>图片会保存到本资料文件夹的 images 目录；保存后按顺序显示在正文上方</small></div><div v-if="images.length" class="image-grid"><figure v-for="(image, i) in images" :key="image.id"><img :src="image.previewUrl" :alt="image.name" /><figcaption>{{ image.name }} <button type="button" @click="removeImage(i)" aria-label="移除图片"><X :size="14" /></button></figcaption></figure></div></section><div class="capture-body"><div class="capture-body-heading"><h3>正文（支持 Markdown）</h3><button class="button button-secondary" type="button" :disabled="polishRunning || !content.trim() || saving || adding" @click="runPolish"><LoaderCircle v-if="polishRunning" class="spin" :size="16" /><Sparkles v-else :size="16" />{{ polishRunning ? '正在整理...' : 'AI 整理为 Markdown' }}</button></div><div v-if="polishRunning || polishText" class="polish-workspace"><div class="polish-pane"><span class="polish-pane-label">原正文</span><textarea v-model="content" aria-label="原正文" rows="16" :disabled="polishRunning" /></div><div class="polish-pane polish-ai-pane"><span class="polish-pane-label">AI 润色结果（Markdown）</span><textarea v-model="polishText" aria-label="AI 润色结果" rows="10" /><article class="markdown-body polish-result-preview" v-html="polishPreview" /></div><p v-if="polishRunning" class="polish-status" role="status"><LoaderCircle class="spin" :size="15" />模型正在处理，原正文保持不变。</p><p v-if="polishError" class="feedback error" role="alert">{{ polishError }}</p><p v-if="polishStale" class="feedback warning" role="status">正文已修改，请重新整理后再采用。</p><div v-if="polishText" class="polish-actions"><button class="button button-secondary" type="button" @click="polishText = ''; polishError = ''">放弃</button><button class="button button-primary" type="button" :disabled="polishStale || saving || adding" @click="adoptPolish"><Check :size="16" />采用整理结果</button></div></div><MarkdownEditor v-else v-model="content" :markdown-path="markdownPath" :preview-title="title" :preview-source-url="sourceUrl" :disabled="saving || adding" @paste-image="onPaste" @paste-images="pasteImages" @choose-images="chooseImages" /></div></form><p v-if="successMessage" class="feedback success">{{ successMessage }}</p><p v-if="errorMessage" class="feedback error">{{ errorMessage }}</p>
  </section>
</template>

<style scoped>
.capture-form{display:grid;gap:20px;max-width:980px}.capture-form>label{display:grid;gap:8px;color:var(--muted-strong);font-size:13px;font-weight:600}.capture-form input{width:100%;padding:11px 12px;color:var(--ink);background:var(--surface);border:1px solid var(--line-strong);border-radius:8px;font:inherit;font-weight:400}.input-icon{display:flex;align-items:center;gap:8px;padding:0 10px;border:1px solid var(--line-strong);border-radius:8px}.input-icon input{border:0;padding-left:0}.capture-body{display:grid;gap:8px}.capture-body-heading{display:flex;align-items:center;justify-content:space-between;gap:12px}.capture-body-heading h3{margin:0;color:var(--ink);font-size:15px}.image-panel{border:1px solid var(--line);border-radius:10px;padding:16px;background:var(--surface-alt)}.image-panel-head{display:flex;justify-content:space-between;margin-bottom:10px}.dropzone{display:flex;flex-wrap:wrap;align-items:center;justify-content:center;gap:10px;padding:22px;border:1px dashed var(--line-strong);border-radius:8px;color:var(--muted-strong)}.dropzone small{width:100%;text-align:center;font-size:11px}.image-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:12px;margin-top:14px}.image-grid figure{margin:0;border:1px solid var(--line);border-radius:8px;overflow:hidden;background:var(--surface)}.image-grid img{display:block;width:100%;height:120px;object-fit:contain}.image-grid figcaption{display:flex;align-items:center;justify-content:space-between;padding:7px;font-size:11px}.image-grid button{border:0;background:transparent;color:var(--muted-strong)}.feedback{margin-top:16px;padding:10px;border-radius:6px}.success{color:var(--success);background:var(--success-soft)}.error{color:#b42318;background:#fff1f0}.warning{color:var(--warning)}.polish-review{display:grid;gap:10px;max-width:980px;padding-top:20px;border-top:1px solid var(--line)}.polish-columns{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.polish-pane{display:grid;gap:7px;min-width:0}.polish-pane-label{color:var(--muted-strong);font-size:12px;font-weight:700}.polish-columns textarea{width:100%;min-height:260px;padding:12px;color:var(--ink);background:var(--surface);border:1px solid var(--line-strong);border-radius:6px;font:14px/1.7 Consolas,monospace;resize:vertical}.polish-columns textarea[readonly]{background:var(--surface-alt);color:var(--muted-strong)}.polish-result-preview{max-height:220px;overflow:auto;padding:12px;background:var(--surface-alt);border:1px solid var(--line)}.polish-actions{display:flex;justify-content:flex-end;gap:8px}.spin{animation:spin 1s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}
