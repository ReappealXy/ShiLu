...
## 2026-09-19：补充 GitHub 首页说明与界面截图

状态：完成，仅修改文档与截图。

新增根目录 `README.md`，介绍当前功能、Windows 安装与使用、可选 AI 配置、本地 Markdown 文件结构、开发环境和构建命令。补充总览、阅读、Markdown 对照编辑三张截图，保存至 `docs/images/`，通过相对路径供 GitHub 展示。

截图使用当前生产构建、独立外部 Chrome、演示资料和模拟桌面接口；未读取私人资料库或真实密钥，未修改产品界面。已逐张检查截图，前端构建通过，截图期间无浏览器运行时异常。README 本地图片与文档链接均已检查。

Git Summary：`docs: 完善项目 README 并添加界面截图`

Git Description：`补充功能介绍、安装使用、AI 配置、本地存储及源码构建说明，添加总览、阅读和 Markdown 编辑三张演示截图。`

大白话：把 README 和截图随项目一起上传后，GitHub 仓库首页就能直接显示软件介绍和使用效果；本次没有上传仓库或发布安装包。
